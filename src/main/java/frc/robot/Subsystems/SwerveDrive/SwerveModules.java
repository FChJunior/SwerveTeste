...
public class SwerveModules {
    ...
    void Spark_ConfigPID_Angulation(CANSparkMax motor, double wrapping) {
        motor.getPIDController().setP(0.3); // Ajuste este valor conforme necessário
        motor.getPIDController().setI(0.0); // Normalmente 0 para começar
        motor.getPIDController().setD(0.1); // Ajuste para controlar oscilações
        motor.getPIDController().setOutputRange(-wrapping, wrapping);
        motor.getPIDController().setPositionPIDWrappingEnabled(true);
        motor.getPIDController().setPositionPIDWrappingMaxInput(21.40); // Certifique-se do limite correto
    ...
    public void SwerveDrivingInit() {
        ...
        angulationMotor1.getEncoder().setPositionConversionFactor(1.0); // Ajuste conforme necessário
        angulationMotor2.getEncoder().setPositionConversionFactor(1.0);
        angulationMotor3.getEncoder().setPositionConversionFactor(1.0);
        angulationMotor4.getEncoder().setPositionConversionFactor(1.0);
    ...
    public void SwerveDriving() {
        ...
        absYaw = absYaw < 0 ? absYaw + 360 : absYaw;
        ...
        if (Analog_Simulated(4, 5, 0.15) && Analog_Simulated(0, 1, 0.15)) {
            if (!var1) {
                ...
                angulationMotor1.getPIDController().setReference(225 / ref_encoder, ControlType.kPosition);
                ...
                angulationMotor4.getPIDController().setReference(45 / ref_encoder, ControlType.kPosition);
            }
        ...
        if (!var1) {
            if (Analog_Simulated(0, 1, 0.15)) {
                ...
                if (player.getRightX() > 0.15 || player.getRightX() < -0.15) {
                    ...
                    if ((rightJoystick_DEG >= -135 || rightJoystick_DEG < -45)
                    ...
                    if ((rightJoystick_DEG >= 45 || rightJoystick_DEG < 135)
                    ...
                    if ((rightJoystick_DEG >= -45 || rightJoystick_DEG < 45)
